...
const Header = () => {
  ...
  const handleApiHealthCheck = () => {
    ...
    response.catch((err) => {
      toast.error(`API connection has not been reacheed: ${err}`);
    });
    response.then((res) => {
    ...
    <header className="w-full flex items-center h-18 md:h-16 p-4 mb-8 border-b shadow-sm shadow-slate-50/60">
        ...
        <Sheet>
          ...
          <SheetContent className="py-16 px-8">
            ...
            <div className="flex flex-col gap-6 capitalize">
              <Link
                href="/"
                className="cursor-pointer text-accent-foreground font-medium"
              >
                ultimos scripts
              </Link>
              <Link
      ...
      <div className="w-full flex items-center justify-between px-8">
        ...
        <div className="flex justify-center items-center">
          <Button onClick={handleApiHealthCheck} className="cursor-pointer">
            <Hexagon />
