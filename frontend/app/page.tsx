...
export default function Home() {
  ...
  const [statusProcess, setStatusProcess] = useState<StatusProcess | null>(null)
  ...
  const handleExecute = async (fileId: string) => {
    ...
    try {
      ...
      toast.success(`${response.message} - Processed Rows: ${response.processed_rows}`)
      fetchStatus(fileId)
      handleResult(fileId);
    } catch (e) {
  ...
  return (
    ...
    <div className="space-y-8 px-16 py-12">
        ...
        <div className="space-y-2">
          <h1 className="text-4xl font-bold tracking-tight">AI CSV Cleaner</h1>
          <p className="text-xl text-muted-foreground max-w-2xl mx-auto">
            Limpe e processe seus dados CSV automaticamente usando inteligência artificial
          </p>
        </div>
      </div>
      <div className="w-full space-y-8">
        <FileUpload selectedFile={file} onFileSelect={handleFileChange} upload={handleUpload} isUploading={isLoading}/>
        {(statusProcess?.file_id || statusProcess) && (
            <ProgressSteps statusProcess={statusProcess} error={error} isLoading={statusLoading}/>
        )}

        {statusProcess?.ready && (
          <div className="">
            <h2 className="text-2xl text-muted-foreground pb-4 font-medium">Tabela com dados Processados</h2>
            <ResultsTable result={result} downloadFile={() => handleDownload(statusProcess?.file_id)}/>
          </div>
        )}
